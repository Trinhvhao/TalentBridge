...
% TikZ styles
\tikzstyle{process} = [rectangle, rounded corners, minimum width=2.5cm, minimum height=1cm, text centered, draw=black, fill=primaryblue!20, font=\small]
\tikzstyle{database} = [cylinder, shape border rotate=90, aspect=0.25, minimum width=2cm, minimum height=1.2cm, text centered, draw=black, fill=secondarygreen!20, font=\small]
\tikzstyle{decision} = [diamond, minimum width=2cm, minimum height=1cm, text centered, draw=black, fill=accentorange!20, font=\small]
\tikzstyle{data} = [trapezium, trapezium left angle=70, trapezium right angle=110, minimum width=2cm, minimum height=1cm, text centered, draw=black, fill=gray!20, font=\small]
\tikzstyle{arrow} = [thick,->,>=stealth]

\begin{document}

\title{TalentBridge Figures Test - RAG Workflow \& Fixed Diagrams}

\author{\IEEEauthorblockN{Test Document}
\IEEEauthorblockA{For reviewing updated figures}}

\maketitle

\section{Introduction}
This document contains all updated figures for TalentBridge paper. Review each figure for clarity and correctness.

\section{System Architecture}

Figure~\ref{fig:architecture} shows the complete system architecture including the new RAG module.
...
\section{CV Parsing Workflow}

Figure~\ref{fig:cv_parsing_flow} illustrates the CV parsing process with LLM-based extraction.
...
\section{Semantic Job Matching}

Figure~\ref{fig:matching_flow} shows the semantic matching workflow with vector similarity.
...
Figure~\ref{fig:rag_workflow} demonstrates the Retrieval-Augmented Generation pipeline for job insights.

% NEW FIGURE 4: RAG Workflow
\begin{figure}[htbp]
\centering
...
All figures have been updated with:
\begin{itemize}
\item Better spacing and alignment
\item Consistent color coding (AI=orange, Vector=green, RAG=blue)
\item Clearer labels and annotations
\item Proper node distances for readability
\item New RAG module integrated into architecture
\end{itemize}
